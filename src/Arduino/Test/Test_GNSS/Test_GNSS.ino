...
// Indikátor, zda již byla přijata zpráva $GNGGA
bool gnggaReceived = false;
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    // pokud řádek začíná na GNGGA a je to poprvé, přiřadíme true do proměnné gnggaReceived
    if (nmeaMessage.startsWith("$GNGGA") && !gnggaReceived) {
      gnggaReceived = true;
    }
    // pokud byla přijata zpráva $GNGGA, vypisujeme čas
    if (gnggaReceived) {
